...
export const db = {
    ...
    async saveFavourite(data) {
        ...
        if (result) {
            return { ...result, start: result.start_location, end: result.end_location }
        }
        return null
    },

    async saveCompletedRide(data) {
        // --- FIXED: Capture and Log Error ---
        const { data: result, error } = await supabase
            .from('rides')
            .insert([{ ...data, status: 'ongoing' }])
            .select()
            .single()
        
        if (error) {
            console.error("Supabase Insert Error:", error) // Check your terminal for this!
            return { error }
        }
